...
Install once (the tensor-train and cross-interpolation kernels come from the sibling
\code{qutecipy} package):
\begin{lstlisting}[language=bash]
pip install -e ../qutecipy --no-deps
pip install numpy scipy jax
\end{lstlisting}
and put the repository directory on \code{sys.path} (the examples do this themselves).
